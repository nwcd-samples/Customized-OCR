function extract_value(blockItemList, item_list){
    ...
    for(var i= 0; i< item_list.length; i++){
        ...
        if(item['recognition_type'] == 'two-space'){

            result = doTwoSpace(item, blockItemList)
            if (result == null){
                continue
            }
            result_item = result[0]
...
function doTwoSpace(item, blockItemList){

    console.log(item['key_word_list'])
    key_start_block_item = null
    key_end_block_item = null
    for(var j=0; j<blockItemList.length; j++){
       blockItemList[j]['text'] = delete_unnecessary_char(blockItemList[j]['text'])
       if(item['key_word_list'][0] == blockItemList[j]['text']){
            key_start_block_item = blockItemList[j]
       }
       if(item['key_word_list'][1] == blockItemList[j]['text']){
            key_end_block_item = blockItemList[j]
       }

       if(key_start_block_item!=null && key_end_block_item!=null){
        break
       }
    }
    ...
    value_block = find_next_right_block_item(blockItemList, key_end_block_item)

    if(value_block !=null ){
        result_item = {}
        result_item['target_name'] = item['target_value_name']
        result_item['value'] = value_block['text']
        result_item['sub_name'] = '-'

        return [result_item, key_end_block_item]
    }else {
        return null
    }
