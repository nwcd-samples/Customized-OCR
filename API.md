...
|RecognitionType|二级|否|enumeration|default|识别类型，可选值为：default、table<br>分别代表要识别的内容是普通的key/value形式、表格形式|
...
| 参数名 | 层级 | 是否必填 | 类型 | 默认值 | 说明 |
| ----  | ---- | ---- | ---- | ---- | ---- |
|MainColumnIndex|二级|否|int|0|主列，用来做行切分的 '列' 的索引|
|MaxRowHeightRatio|二级|否|float|2.5|用来迭代查找的行高，以headColumn * MaxRowHeightRatio|
|MaxRowCount|二级|否|int|20|最大行数，超过的丢弃|
|Columns|二级|是|list|无|定义所有列|
|ColumnName|三级|是|string|无|列名|
|IndexType|三级|否|enumeration|default|列类型，可选值为：default、begin、end<br>通过begin、end两个列名对表格的表头进行定位，任意选2列即可|
|KeyWordList|三级|否|list|无|列头可能出现的关键字|
|MarginLeftType|三级|否|int|2|可选值为1、2、3<br>1、以当前Column的左边作为分界线，范围最近<br>2、以当前Column和左边Column的中点作为分界线，范围适中<br>3、以左边Column的右边作为分界线，范围最远|
|MoveLeftRatio|三级|否|float|0.0|左分界线在MarginLeftType基础上移动的比率，向左为负，向右为正。<br>移动距离为headColumn.width * MoveLeftRatio|
|MarginRightType|三级|否|int|2|可选值为1、2、3<br>1、以当前Column的右边作为分界线，范围最近<br>2、以当前Column和右边Column的中点作为分界线，范围适中<br>3、以右边Column的左边作为分界线，范围最远|
